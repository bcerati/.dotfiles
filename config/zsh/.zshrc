...
if [ "$color_prompt" = yes ]; then
    phpVersion=$(php -v | grep ^PHP | cut -d' ' -f2 | cut -d'.' -f1,2)
    phpVersion="php v$phpVersion"

    nodeVersion=$(node -v | cut -d'v' -f2 | cut -d"." -f'1,2')
    nodeVersion="node v$nodeVersion"

    PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)──}$(venv_info)(%B%F{%(#.red.blue)}%n%(#..@%m)(%F{%(#.blue.red)}$phpVersion%F{%(#.red.blue)} | %F{%(#.red.cyan)}$nodeVersion%F{%(#.red.blue)})%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}$(git_branch)]\n└─── %B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
...
alias luamake=/home/boris/tools/lua-language-server/3rd/luamake/luamake
